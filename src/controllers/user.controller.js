...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (
    [username, email, password, phoneNumber, role].some(
      (fields) => fields?.trim() === ""
    )
  ) {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, RegisteredUser, "user successfully registered"));
});
...
const refresh_RefreshToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (incomingTOKEN !== user.refreshToken) {
      throw new ApiError(
        401,
        "refreshToken didn't match with user refreshToken"
      );
    }
    ...
    throw new ApiError(
      500,
      error || "something went wrong while  Refreshing refreshToken "
    );
  }
});
...
const changePassword = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, {}, "user change Passsword Successfully"));
  } catch (error) {
    throw new ApiError(
      500,
      error || "something went wrong while user change Password"
    );
  }
});
const changeUserDetails = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, user, "user change Passsword Successfully"));
  } catch (error) {
    throw new ApiError(
      500,
      error || "something went wrong while user change Password"
...
const getCurrentUser = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, userProfile, "successfully get user Profile"));
});
